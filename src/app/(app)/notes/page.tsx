...
// Soft tints that rotate across cards for a sticky-note feel
const NOTE_TINTS = [
    ...
    <div className="p-4">
      ...
      <div className="mt-4 border border-border rounded-lg overflow-hidden">
        <textarea
          ...
          className="w-full px-4 py-3 text-sm bg-card resize-none outline-none min-h-[80px] placeholder:text-muted-foreground"
          rows={3}
        />
        <div className="flex items-center justify-between px-4 py-2 border-t border-border bg-muted/50">
          ...
          <button
            onClick={handleSave}
            disabled={!draft.trim()}
            className="text-xs font-medium px-3 py-1.5 rounded-md bg-accent text-accent-foreground disabled:opacity-40 transition-opacity"
          >
            Save
          </button>
        </div>
                  ...
                  {editingId === note.id ? (
                    <div>
                      <textarea
                        ...
                        className="w-full text-sm bg-transparent resize-none outline-none min-h-[60px]"
                      />
